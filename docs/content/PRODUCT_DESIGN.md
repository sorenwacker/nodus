...
**Chat transcript (required behavior):** The agent bar is a chat window, not a fire-and-forget input. The bar sits at the top of the canvas, so its input row stays pinned there and a scrollable transcript grows downward beneath it, capped at 45% of the window height; the bar itself grows with the transcript rather than clipping it to the input row's height. The transcript shows the exchange in order: each prompt the user sent, and each answer the agent gave in full. Whenever the bar is visible the transcript area is visible too: before the first exchange it holds a placeholder naming itself as where answers appear, because an area that materializes only once output exists leaves the same question - where does the output go - unanswered. An answer that arrives as plain text is displayed as written - never truncated - because otherwise a reply that performs no canvas action leaves no visible trace at all.
