...
| Closes with an upward push | The sheet opened by moving down, so it closes by moving up: a push against the top screen edge closes it. Mirrors the spatial model (time below, graph above); the left edge is reserved for stepping back through the storyline layers |
